...
class NdtdpiController extends Controller
{
    ...
    public function searchDPI(Request $request){
        if ($request->quote === "empty"){
            $c = Dpi::all();
        } else{
            $c = Dpi::where('CRET_NO', $request->quote)->get();
        }
        return response()->json($c);
    }
    public function editDPI(Request $request){
        if ($request->id !== ""){
            $dpi = Dpi::where('_id', $request->id)->first();
            if ($dpi !== null){
                $dpi->ID_WO = $request->work;
                $dpi->CERT_NO = $request->cert;
                $dpi->REQ_NO = $request->req;
